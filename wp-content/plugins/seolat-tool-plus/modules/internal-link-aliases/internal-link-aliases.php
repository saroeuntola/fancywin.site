...
class SL_InternalLinkAliases extends SL_Module {
	...
	function init() {
		// js scripts
		add_action( 'admin_enqueue_scripts', array($this, 'mask_generator_add_js'));
		// ============================================================================

		add_filter('sl_custom_update_postmeta-aliases', array(&$this, 'save_post_aliases'), 10, 4);
		add_filter('sl_get_setting-internal-link-aliases-alias_dir', array(&$this, 'filter_alias_dir'));
		
		if (lat_wp::permalink_mode() == LATWP_PRETTY_PERMALINKS) {
			...
			add_action('sl_do_robotstxt', array(&$this, 'block_aliases_dir'));
		}
	}

	// add js script
	function mask_generator_add_js($hook) {
		wp_enqueue_script( 'mask_generator', plugin_dir_url( __FILE__ ).'mask-generator-script.js', array('jquery'));
	}
	...
	static function get_module_title() { return __('Link Mask Generator', 'seolat-tool-plus'); }
	static function get_menu_title() { return __('Link Mask Generator', 'seolat-tool-plus'); }
	...
	function admin_page_contents() {
		...
		if (lat_wp::permalink_mode() != LATWP_PRETTY_PERMALINKS)
			$this->print_message('error', sprintf(__('Link Mask Generator won&#8217;t work with default or &#8220;pathinfo&#8221; permalinks. Please change your <a href="%s">permalink structure</a> to enable this module&#8217;s functionality.', 'seolat-tool-plus'), 'options-permalink.php'));
	...
	function get_admin_page_tabs() {
		return array(
			  array('id' => 'aliases',  'title' => __('Aliases', 'seolat-tool-plus'),  'callback' => 'editor_tab')
			, array('id' => 'settings', 'title' => __('Settings', 'seolat-tool-plus'), 'callback' => 'settings_tab')
		);
	}
	...
	function editor_tab() {
		...
		if ($num_aliases > 0) {
			$this->admin_subheader(__('Edit Existing Aliases', 'seolat-tool-plus'));
			$this->aliases_form(0, $aliases);
		}
		
		$this->admin_subheader(__('Add a New Alias', 'seolat-tool-plus'));
	...
	function aliases_form($start_id = 0, $aliases, $existing_item = true) {
		...
		//Set headers
		$headers = array(
			  'alias-from' => __('Actual URL', 'seolat-tool-plus')
			, 'alias-to' => __('Alias URL', 'seolat-tool-plus')
			, 'alias-posts' => __('Only on This Post&hellip; <em>(optional)</em>', 'seolat-tool-plus')
		);
		if ($existing_item) $headers['alias-delete'] = __('<input type="checkbox" class="check my-check" /> Delete', 'seolat-tool-plus');
		...
		foreach ($aliases as $id => $alias) {
			...
			$test_link = $existing_item ? "<td class='sl-alias-to-test'>[<a href='$alias_url' target='_blank'>" . __('Test', 'seolat-tool-plus') . "</a>]</td>" : '';
			...
			$cells = array(
				...
				, 'alias-to' => "
<table><tr>
	<td class='sl-alias-to-dir'>/$alias_dir/</td>
	<td class='sl-alias-to-slug'>" . $this->get_input_element('textbox', "alias_{$i}_to", $alias['to']) . "</td>
	$test_link
</tr></table>"
	...
	function settings_tab() {
		...
		$this->textbox('alias_dir', __('Alias Directory', 'seolat-tool-plus'), $this->get_default_setting('alias_dir'));
		if ($this->plugin->module_exists('link-nofollow'))
			$this->checkbox('nofollow_aliased_links', __('Nofollow masked links', 'seolat-tool-plus'), __('Link Attributes', 'seolat-tool-plus'));
		$this->admin_form_table_end();
	}
	
	function filter_alias_dir($alias_dir) {
		return trim(lat_string::preg_filter('a-zA-Z0-9_/', $alias_dir), '/');
	}
	...
	function postmeta_fields($fields, $screen) {
		...
		if ($content && preg_match_all('@ href=["\']([^#][^"\']+)["\']@', $content, $matches)) {
			...
			$html = "<tr valign='top'>\n<th scope='row' class='sl'>".__('Link Masks:', 'seolat-tool-plus')."</th>\n<td>\n";
			
			$html .= "<table class='widefat'><thead>\n";
			$headers = array(__('URL', 'seolat-tool-plus'), '', __('Mask URL', 'seolat-tool-plus'));
			...
			foreach ($urls as $url) {
				...
				$ht_url = esc_html(lat_string::truncate($url, 30));
				...
				$html .= "<tr><td><a href='$a_url' title='$a_url' target='_blank'>$ht_url</a><input type='hidden' name='_sl_aliases[$alias_id][from]' value='$a_url' /></td>\n<td>&rArr;</td><td>/$alias_dir/<input type='text' name='_sl_aliases[$alias_id][to]' value='$a_alias_to' /></td></tr>\n";
			}
			
			$html .= "</tbody>\n</table>\n";
			
			$html .= '<p><small>' . __('You can stop search engines from following a link by typing in a mask for its URL.', 'seolat-tool-plus') . "</small></p>\n";
	...
	function redirect_aliases() {
		...
		foreach ($aliases as $alias)
			if ($to = $alias['to'])
				if (lat_url::equal(lat_url::current(), get_bloginfo('url') . "/$alias_dir/$to/"))
					wp_redirect($alias['from']);
	}
	
	function block_aliases_dir() {
		echo '# ';
		_e("Added by SEOLAT Tool Plus Link Mask Generator module", 'seolat-tool-plus');
		echo "\n";
		...
		echo '# ';
		_e('End Link Mask Generator output', 'seolat-tool-plus');
		echo "\n\n";
	}
	...
	function add_help_tabs($screen) {
		...
		$screen->add_help_tab(array(
			  'id' => 'sl-internal-link-aliases-overview'
...
", 'seolat-tool-plus')));
		...
		$screen->add_help_tab(array(
			  'id' => 'sl-internal-link-aliases-aliases'
			, 'title' => __('Aliases Tab', 'seolat-tool-plus')
			, 'content' => __("
...
", 'seolat-tool-plus')));
		...
		$screen->add_help_tab(array(
			  'id' => 'sl-internal-link-aliases-settings'
