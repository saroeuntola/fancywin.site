...
class SL_Slugs extends SL_Module {
	
	static function get_module_title() { return __('Slug Optimizer', 'seolat-tool-plus'); }
	...
	function admin_page_contents() {
		...
		$this->textarea('words_to_remove', __('Words to Remove', 'seolat-tool-plus'), 20);
	...
	function optimize_slug($slug) {
		...
		$slug = lat_string::tolower(stripslashes($slug));
		$words = lat_string::tolower(stripslashes($this->get_setting('words_to_remove')));
		
		//Remove the stopwords from the slug
		$newslug = implode("-", array_diff(explode(" ", $slug), lat_array::explode_lines($words)));
