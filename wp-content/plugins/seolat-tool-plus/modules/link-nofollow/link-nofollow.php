...
class SL_LinkNofollow extends SL_Module {
	
	static function get_module_title() { return __('Nofollow Manager', 'seolat-tool-plus'); }
	...
	function admin_page_contents() {
		...
		$this->checkboxes(array(
				  'nofollow_adjacent_post' => __('Adjacent post links (next post / previous post)', 'seolat-tool-plus')
				, 'nofollow_category_loop' => __('Category links (after posts)', 'seolat-tool-plus')
				, 'nofollow_category_list' => __('Category links (in lists)', 'seolat-tool-plus')
				, 'nofollow_comment_popup' => __('Comment anchor links', 'seolat-tool-plus')
				, 'nofollow_comment_feed' => __('Comment feed links', 'seolat-tool-plus')
				, 'nofollow_date_archive' => __('Date-based archive links', 'seolat-tool-plus')
				, 'nofollow_paged' => __('Pagination navigation links (all)', 'seolat-tool-plus')
				, 'nofollow_paged_home' => __('Pagination navigation links (on blog home only)', 'seolat-tool-plus')
				, 'nofollow_post_more' => __('&#8220;Read more&#8221; links', 'seolat-tool-plus')
				, 'nofollow_register' => __('Registration link', 'seolat-tool-plus')
				, 'nofollow_login' => __('Login link', 'seolat-tool-plus')
				, 'nofollow_tag_loop' => __('Tag links (after posts)', 'seolat-tool-plus')
				, 'nofollow_tag_list' => __('Tag links (in lists and clouds)', 'seolat-tool-plus')
			), __('Add the nofollow attribute to...', 'seolat-tool-plus'));
	...
	function postmeta_fields($fields, $screen) {
		...
		if (strcmp($screen, 'page') == 0)
			$fields['links'][30]['nofollow'] = $this->get_postmeta_checkbox('nofollow', __('When displaying page lists, nofollow links to this page', 'seolat-tool-plus'), __('Nofollow:', 'seolat-tool-plus'));
	...
	function add_help_tabs($screen) {
		...
		$screen->add_help_tab(array(
			  'id' => 'sl-link-nofollow-overview'
			, 'title' => $this->has_enabled_parent() ? __('Nofollow Manager', 'seolat-tool-plus') : __('Overview', 'seolat-tool-plus')
			, 'content' => __("
