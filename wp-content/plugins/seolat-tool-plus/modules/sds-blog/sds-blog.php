...
class SL_SdsBlog extends SL_Module {
	
	static function get_module_title() { return __('SEO News', 'seolat-tool-plus'); }
	
	static function get_menu_title() { return __('SEO News', 'seolat-tool-plus'); }

	function get_page_title() { return __('SEO LAT News', 'seolat-tool-plus'); }	
	...
	function __construct() {
		add_filter('sl_settings_export_array', array(&$this, 'filter_export_array'));
	}
	...
	function load_blog_rss() {
		$rss = lat_wp::load_rss('https://affiliatecms.com/seo-news/feed/', SL_USER_AGENT);
	...
	function admin_page_contents() {
		...
		echo "<a href='https://affiliatecms.com'><img src='{$this->plugin->plugin_dir_url}plugin/img/aff-logo.png' alt='".__('SEO LAT', 'seolat-tool-plus')."' id='sds-logo' width=\"300\"/></a>";
		echo "<p>".__('Get the latest SEO news, updates, tips to optimize SEO for Google, Bing, and other search engines: <a href="https://affiliatecms.com/seo-news/" target="_blank">SEO News</a>.', 'seolat-tool-plus')."</p>\n";
		echo "<div class='rss-widget'>\n";
		
		add_filter('http_headers_useragent', 'sl_get_user_agent');
		add_filter('esc_html', array(&$this, 'truncate_at_ellipsis'));
		$this->sds_blog_rss_output( 'https://affiliatecms.com/seo-news/feed/', array('show_summary' => 1, 'show_date' => 1, 'items' => 10) );
		remove_filter('esc_html', array(&$this, 'truncate_at_ellipsis'));
		remove_filter('http_headers_useragent', 'sl_get_user_agent');
	...
	function truncate_at_ellipsis($content) {
		$end = '[...]';
		if (lat_string::has($content, $end)) {
			$content = lat_string::upto($content, $end);
			$content = lat_string::rtrim_substr($content, $end);
		}
		return lat_string::endwith($content, '[&hellip;]');
	}
