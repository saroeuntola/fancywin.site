...
class SL_FooterAutolinks extends SL_Module {
	...
	static function get_module_title() { return __('Footer Deeplink Juggernaut', 'seolat-tool-plus'); }
	function get_module_subtitle() { return __('Footer Links', 'seolat-tool-plus'); }
		...
		foreach ($links as $link_data) {
			...
			switch ($from_genus) {
				...
				case 'taxonomy':
					...
					if (	    lat_wp::is_tax($from_type, $from_id) //Is
				...
				case 'url':
					...
					if ($from_id) {
						if (	    lat_url::equal(lat_url::current(), $from_id)
								|| ($from_match_children && lat_string::startswith(lat_url::current(), $from_id))
								)
							$is_from = !$from_match_negative;
			...
			$title  = strlen($a_titletext = sl_esc_attr($link_data['title'])) ? " title=\"$a_titletext\"" : '';
			
			$a_url = sl_esc_attr($this->jlsuggest_value_to_url($link_data['to']));
	...
	function footer_links_form($start_id = 0, $links, $delete_option = true) {
		...
		//Set headers
		$headers = array(
			  'link-from' => __('Link Location <em>(optional)</em>', 'seolat-tool-plus')
			, 'link-from-match' => ''
			, 'link-anchor' => __('Anchor Text', 'seolat-tool-plus')
			, 'link-to' => __('Destination', 'seolat-tool-plus')
			, 'link-title' => __('Title Attribute <em>(optional)</em>', 'seolat-tool-plus')
			, 'link-options' => __('<input type="checkbox" class="check my-check" /> Options', 'seolat-tool-plus')
		);
		if ($delete_option) $headers['link-delete'] = __('Delete', 'seolat-tool-plus');
		...
		foreach ($links as $link) {
			...
			$cells = array(
				...
				, 'link-from-match' =>
					 $this->get_input_element('checkbox', "footer_link_{$i}_from_match_children", $link['from_match_children'] == 1, str_replace(' ', '&nbsp;', __('Match child content', 'seolat-tool-plus')))
					.'<br />'
					.$this->get_input_element('checkbox', "footer_link_{$i}_from_match_negative", $link['from_match_negative'] == 1, str_replace(' ', '&nbsp;', __('Negative match', 'seolat-tool-plus')))
				/*, 'link-from-match' => $this->get_input_element('dropdown', "footer_link_{$i}_from_match", $link['from_match'], array(
					  'is' => __('If this:', 'seolat-tool-plus')
					, 'is,in' => __('If this or child:', 'seolat-tool-plus')
					, '!:is' => __('If not this:', 'seolat-tool-plus')
					, '!:is,in' => __('If not this and not child:', 'seolat-tool-plus')
				))*/
				...
				, 'link-options' =>
					 $this->get_input_element('checkbox', "footer_link_{$i}_nofollow", $link['nofollow'], str_replace(' ', '&nbsp;', __('Nofollow', 'seolat-tool-plus')))
					.'<br />'
					.$this->get_input_element('checkbox', "footer_link_{$i}_target", $link['target'] == 'blank', str_replace(' ', '&nbsp;', __('New window', 'seolat-tool-plus')))
			);
			if ($delete_option)
