...
class SL_ContentAutolinks extends SL_Module {
	...
	static function get_module_title() { return __('Content Deeplink Juggernaut', 'seolat-tool-plus'); }
	function get_module_subtitle() { return __('Content Links', 'seolat-tool-plus'); }
	
	function init() {
		// js scripts
		add_action( 'admin_enqueue_scripts', array($this, 'juggernaut_add_js'));
		// ============================================================================

		add_filter('the_content', array(&$this, 'autolink_content'), self::filter_autolink_content_priority());
		
		add_filter('sl_postmeta_help', array(&$this, 'postmeta_help'), 35);
		add_filter('sl_get_postmeta-autolinks', array(&$this, 'get_post_autolinks'), 10, 3);
		add_filter('sl_custom_update_postmeta-autolinks', array(&$this, 'save_post_autolinks'), 10, 4);
		
		add_filter('sl_get_setting-autolinks-linkfree_tags', array(&$this, 'filter_linkfree_tags'));
		add_filter('sl_get_setting-autolinks-dampen_sitewide_lpa_value', array(&$this, 'filter_dampen_sitewide_lpa_value'));
	}

	// add js script
	function juggernaut_add_js($hook) {
		wp_enqueue_script( 'juggernaut', plugin_dir_url( __FILE__ ).'juggernaut-script.js', array('jquery'));
	}
	...
	function filter_linkfree_tags($tags) {
		return lat_string::preg_filter('a-z0-9,', strtolower($tags));
	}
	
	function filter_dampen_sitewide_lpa_value($value) {
		$value = lat_string::to_int($value);
	...
	function autolink_content($content) {
		...
		lat_array::vklrsort($links, 'anchor');
		$links = array_values($links);
		
		//Dummy vars; needed for PHP4 compat
		$count = 0;
		$link_count = array();
		$content = $this->_autolink_content(lat_wp::get_post_id(), $content, $links, $this->get_setting('limit_lpp_value', 5), $count, $link_count);
	...
	function _autolink_content($id, $content, $links, $limit, &$count, &$link_count, $round=1, $linked_urls=array(), $context='the_content') {
		...
		if ($dest_limit) {
			//$from_post_type_taxonomies = lat_wp::get_object_taxonomy_names($from_post_type);
			$from_post_type_taxonomies = lat_wp::get_taxonomy_names($from_post_type);
		...
		foreach ($links as $data) {
			...
			if (strlen(trim($anchor)) && $to_id !== 0 && $to_id != 'http://') {
				...
				if (lat_string::startswith($type, 'posttype_')) {
					$to_id = intval($to_id);
					...
					if (count($dest_limit_taxonomies)) {
						...
						foreach ($dest_limit_taxonomies as $dest_limit_taxonomy) {
							$from_terms = lat_array::flatten_values(get_the_terms(null, $dest_limit_taxonomy), 'term_id');
							
							if (is_object_in_taxonomy($to_post, $dest_limit_taxonomy))
								$to_terms = lat_array::flatten_values(get_the_terms($to_id, $dest_limit_taxonomy), 'term_id');
							else
								$to_terms = array();
				...
				if (!$this->get_setting('enable_current_url_links', false) && lat_url::equal($url, lat_url::current()))
					continue;
				
				if (!$this->get_setting('enable_self_links', false) && is_singular() && lat_url::equal($url, get_permalink()))
					continue;
				...
				$title	= strlen($titletext = sl_esc_attr($data['title'])) ? " title=\"$titletext\"" : '';
				$class  = $autolink_class ? ' class="' . sl_esc_attr($autolink_class) . '"' : '';
				...
				$lpa_lpu_limits = count($lpa_lpu_limits) ? lat_number::lowest($lpa_lpu_limits) : -1;
				
				$content = lat_string::htmlsafe_str_replace($h_anchor, $link, $content, $limit_enabled ? 1 : $lpa_lpu_limits, $new_count, $this->get_linkfree_tags());
	...
	function admin_page_contents() {
		...
		echo "\n<p>";
		_e('The Content Links section of Deeplink Juggernaut lets you automatically link a certain word or phrase in your post/page content to a URL you specify.', 'seolat-tool-plus');
		echo "</p>\n";
		...
		if ($this->is_action('update')) {
			...
			for ($i=0; $i <= $num_links; $i++) {
				...
				if (lat_string::startswith($to, 'obj_')) {
					$to = lat_string::ltrim_str($to, 'obj_');
					$to = explode('/', $to);
					if (count($to) == 2) {
						$to_type = $to[0];
						$to_id = $to[1];
				...
				$dampen_sitewide_lpa = lat_string::preg_filter('0-9', strval($_POST["link_{$i}_dampen_sitewide_lpa"]));
				$dampen_sitewide_lpa = ($dampen_sitewide_lpa === '') ? false : intval($dampen_sitewide_lpa);
				
				$sitewide_lpa = isset($_POST["link_{$i}_sitewide_lpa"]) ? lat_string::preg_filter('0-9', strval($_POST["link_{$i}_sitewide_lpa"])) : '';
	...
	function content_links_form($guid, $start_id = 0, $links, $delete_option = true) {
		...
		//Set headers
		$headers = array();
		$headers['link-anchor'] = __('Anchor Text', 'seolat-tool-plus');
		$headers['link-to'] = __('Destination', 'seolat-tool-plus');
		$headers['link-title'] = __('Title Attribute <em>(optional)</em>', 'seolat-tool-plus');
		if ($this->get_setting('enable_perlink_dampen_sitewide_lpa', false, null, true) || $this->legacy_sitewide_lpa_in_use)
			$headers['link-dampen-sitewide-lpa'] = __('Dampener', 'seolat-tool-plus');
		if ($this->legacy_sitewide_lpa_in_use)
			$headers['link-sitewide-lpa'] = __('Site Cap', 'seolat-tool-plus');
		$headers['link-options'] = __('<input type="checkbox" class="check my-check" /> Options', 'seolat-tool-plus');
		if ($delete_option)
			$headers['link-delete'] = __('Delete', 'seolat-tool-plus');
		...
		foreach ($links as $link) {
			...
			$cells['link-options'] .=
					 $this->get_input_element('checkbox', "link_{$i}_nofollow", $link['nofollow'], str_replace(' ', '&nbsp;', __('Nofollow', 'seolat-tool-plus')))
					.'<br />'
					.$this->get_input_element('checkbox', "link_{$i}_target", $link['target'] == 'blank', str_replace(' ', '&nbsp;', __('New window', 'seolat-tool-plus')));
			if ($delete_option)
	...
	function save_post_autolinks($false, $value, $metakey, $post) {
		...
		$new_anchors = lat_array::explode_lines($value);
