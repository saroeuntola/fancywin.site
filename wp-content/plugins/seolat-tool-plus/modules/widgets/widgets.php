...
class SL_Widgets extends SL_Module {
	
	static function get_module_title() { return __('SEOLAT Tool Plus Widgets', 'seolat-tool-plus'); }
	...
	function register_widgets() {
		register_widget('SL_Widget_SiloedTerms');
		
		if ($this->plugin->module_exists('footer-autolinks'))
			register_widget('SL_Widget_FooterLinks');
	}
}
...
class SL_Widget_SiloedTerms extends WP_Widget {
	
	function __construct() {
		$widget_ops = array( 'description' => __( "On category archives, displays a list of child categories and/or posts in the category. Displays a list of top-level categories everywhere else. Powered by the SEOLAT Tool Plus plugin.", 'seolat-tool-plus' ) );
		parent::__construct('sl_siloed_terms', __('Siloed Categories', 'seolat-tool-plus'), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		if (lat_wp::is_tax($current_taxonomy)) {
...
?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e( 'Title:', 'seolat-tool-plus' ); ?></label>
		...
		<label for="<?php echo $this->get_field_id('count'); ?>"><?php _e( 'Show post counts', 'seolat-tool-plus' ); ?></label>		
		...
		<label for="<?php echo $this->get_field_id('use_desc_for_title'); ?>"><?php _e( 'Use term descriptions in title attributes', 'seolat-tool-plus' ); ?></label></p>
		
		<p><label for="<?php echo $this->get_field_id('taxonomy'); ?>"><?php _e('Taxonomy:', 'seolat-tool-plus') ?></label>
...
class SL_Widget_FooterLinks extends WP_Widget {
	
	function __construct() {
		$widget_ops = array( 'description' => __( "Add this widget to display Deeplink Juggernaut&#8217;s Footer Links in a widget area of your choosing rather than the default wp_footer section. Powered by the SEOLAT Tool Plus plugin.", 'seolat-tool-plus' ) );
		parent::__construct('sl_footer_autolinks', __('Footer Links', 'seolat-tool-plus'), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		if ($seo_lat_tool_plus->call_module_func('footer-autolinks', 'autolink_footer', $unused, $args))
			$seo_lat_tool_plus->set_module_var('footer-autolinks', 'already_outputted', true);
	...
	function form( $instance ) {
		...
		$display = empty($instance['display']) ? 'list' : $instance['display'];
?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e( 'Title: <em>(optional)</em>', 'seolat-tool-plus' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></p>
		
		<p>
			<label><input type="radio" name="<?php echo $this->get_field_name('display'); ?>" value="list" <?php checked('list', $display); ?>/> <?php _e('Display as a list', 'seolat-tool-plus'); ?></label><br />
			<label><input type="radio" name="<?php echo $this->get_field_name('display'); ?>" value="usersettings" <?php checked('usersettings', $display); ?>/> <?php
				$seo_lat_tool_plus->call_module_func('footer-autolinks-settings', 'get_admin_url', $formats_url);
				printf(__('Use my <a href="%s" target="_blank">footer link HTML formats</a>', 'seolat-tool-plus'), $formats_url);
			?></label>
		</p>
<?php
	}
