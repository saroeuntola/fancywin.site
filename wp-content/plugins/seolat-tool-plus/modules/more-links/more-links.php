...
class SL_MoreLinks extends SL_Module {
	
	static function get_module_title() { return __('More Link Customizer', 'seolat-tool-plus'); }
	...
	function init() {
		...
		add_filter('sl_get_postmeta-morelinktext', array(&$this, 'get_morelinktext_postmeta'), 10, 3);
	}
	
	function admin_page_contents() {
		$this->child_admin_form_start();
		$this->textbox('default', __('Default More Link Text', 'seolat-tool-plus'), $this->get_default_setting('default'));
	...
	function more_link_filter($link, $text=false) {
		...
		if (strlen($newtext = trim($this->get_postmeta('morelinktext'))) || strlen(trim($newtext = $default))) {
			$newtext = str_replace('{post}', sl_esc_html(get_the_title()), $newtext);
	...
	function postmeta_fields($fields, $screen) {
		...
		if (strcmp($screen, 'post') == 0)
			$fields['links'][20]['morelinktext'] = $this->get_postmeta_textbox('morelinktext', __('Anchor Text of &#8220;More&#8221; Link:', 'seolat-tool-plus'));
...
<ul>
	<li>
		...
		<p>If you&#8217;ve already specified custom anchors via WordPress&#8217;s method, SEOLAT Tool Plus will import those anchors automatically into the More Link Customizer.</p>
	</li>
</ul>
", 'seolat-tool-plus');
