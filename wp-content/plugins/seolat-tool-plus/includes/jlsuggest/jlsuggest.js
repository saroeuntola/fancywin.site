jQuery(document).ready( function($) {
	$('input.jlsuggest', '.sdf-admin,#sl-postmeta-box').each(function() {
		var params = $(this).attr('sl:params') ? '&' + $(this).attr('sl:params') : '';
		$(this).jlsuggest(ajaxurl + '?action=sl-jlsuggest-autocomplete' + params,
				...
				if (item) {
					...
					if (item.isheader)
						...
						html = '<li'
							...
							+ ' sl:value="' + Encoder.htmlEncode(item.value || '', true) + '"'
							+ ' sl:selectedtext="' + Encoder.htmlEncode(item.selectedtext || '', true) + '"'
							+ '>' + html + '</li>';
					}
					lis[lis.length] = html;
				}
			...
			if ($currentResult) {
				if (options.textDest) {
					$input
						...
						.html($currentResult.attr('sl:selectedtext') || $currentResult.text()) //Put the selected item into the selection box
						.parentsUntil('tr') //If we're in a table...
						.next('td')
						.children('input')
						.focus(); //...then focus the next textbox in the table
					
					$input.val($currentResult.attr('sl:value'));
				} else {
