...
class lat_array {
	...
	static function vksort(&$arr, $valuekey) {
		$valuekey = lat_string::preg_filter('A-Za-z0-9 ', $valuekey);
	 ...
	 * $singular_names = lat_array::flatten_values($post_types, array('labels', 'singular_name'));
	 * ---
	 * 
	...
	static function flatten_values($arr, $value_keys, $use_default_if_empty=false, $default='') {
		foreach ((array)$value_keys as $key)
			$arr = lat_array::_flatten_values($arr, $key, $use_default_if_empty, $default);
		return $arr;
	}
	...
	static function key_replace($array, $key_changes, $recursive = true, $return_replaced_only = false) {
		...
		foreach ($array as $key => $value) {
			...
			if ($recursive && is_array($value)) {
				$oldvalue = $value;
				$value = lat_array::key_replace($value, $key_changes, true, $return_replaced_only);
	...
	static function value_replace($array, $value_changes, $recursive = true, $return_replaced_only = false) {
		...
		foreach ((array)$array as $key => $value) {
			...
			if ($recursive && is_array($value))
				$value = lat_array::value_replace($value, $value_changes, true);
	...
	static function simplify($arr, $keyloc, $valloc, $use_default_if_empty=false, $default='') {
		$keys = lat_array::flatten_values($arr, $keyloc, $use_default_if_empty, $default);
		$values = lat_array::flatten_values($arr, $valloc, $use_default_if_empty, $default);
