...
class lat_io {
	...
	static function is_file($filename, $path, $ext=false) {
		$is_ext = strlen($ext) ? lat_string::endswith($filename, '.'.ltrim($ext, '*.')) : true;		
		return is_file(lat_io::tslash($path).$filename) && $is_ext;
	}
	...
	static function is_dir($name, $path) {
		return $name != '.' && $name != '..' && is_dir(lat_io::tslash($path).$name);
	}
	...
	static function print_csv($csv) {
		...
		foreach ($csv as $row) {
			...
			foreach ($headers as $header) {
				...
				if (lat_string::has($csv_row[$header], ',')) $csv_row[$header] = '"'.$csv_row[$header].'"';
			}
